...
const stationaryShops: Stationary[] = [
  ...
  },
  {
    ...
    services: ["Yüksek kalite baskı", "Resmi okul kitapları", "All kinds of stationary"]
  },
  {
    id: 3,
                ...
                <ul className="list-disc list-inside space-y-2">
                  {shop.services.map((service, index) => (
                    <li key={index} className="text-sm text-gray-700 flex items-center">
                      {service.toLowerCase().includes('baskı') && <Printer className="w-4 h-4 mr-2" />}
                      {service.toLowerCase().includes('kitap') && <Book className="w-4 h-4 mr-2" />}
                      {service}
                    </li>
                  ))}
                </ul>
