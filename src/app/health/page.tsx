...
export default function HealthPage() {
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <h1 className="text-4xl font-bold mb-8 text-gray-900">Health Facilities</h1>
        ...
        {healthFacilities.map((facility) => (
          <div key={facility.id} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 p-6 border border-gray-200">
            <h2 className="text-2xl font-semibold mb-4 flex items-center text-gray-900">
              {facility.type === 'Hospital' && <Building2 className="w-6 h-6 mr-2 text-red-500" />}
              {facility.type === 'Clinic' && <Stethoscope className="w-6 h-6 mr-2 text-blue-500" />}
              {facility.type === 'Pharmacy' && <Pill className="w-6 h-6 mr-2 text-green-500" />}
              {facility.name}
            </h2>
      ...
      <div className="mt-10 p-6 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 rounded-lg shadow-md">
        ...
        <p className="mt-2 text-gray-700">
          ...
          <Link href="https://www.kteb.org/dp/?lang=en" className="text-blue-600 hover:underline ml-1">
