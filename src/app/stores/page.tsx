  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {stores.map((store) => (
          <div key={store.id} className="bg-white rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-300 p-6 border border-gray-200">
            <h2 className="text-2xl font-semibold mb-4 flex items-center text-gray-800">
              {store.type === 'Technology' && <Laptop className="w-6 h-6 mr-3 text-blue-500" />}
              {store.type === 'Clothes' && <Shirt className="w-6 h-6 mr-3 text-pink-500" />}
              {store.type !== 'Technology' && store.type !== 'Clothes' && <Tag className="w-6 h-6 mr-3 text-yellow-500" />}
              {store.name}
            </h2>
